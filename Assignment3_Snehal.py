...
def main():
    ...
    b_counts = (
        ...
        .option(
            "dbtable",
            "(select Hit,Atbat,batter,bc.game_id,(g.local_date) as l_dt \
    ...
    # results = spark.sql(
    # """SELECT SUM(bc_i.Hit) as Sum_H,SUM(bc_i.atbat)\
    #  as Sum_A,bc_o.batter,bc_o.game_id \
    ...
    # )
    batting_avg_cal = BattingAvgCalc(
        inputCols=["Hit", "atbat"], outputCol="rolling_average"
    )
    # rolling_avg = batting_avg_cal.transform(results)
    rolling_avg = batting_avg_cal.transform(b_counts)
